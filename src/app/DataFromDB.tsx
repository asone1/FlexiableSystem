import { getArgByIndex } from './components/Utilities';
import axios from 'axios';

var CONFIG = require('../resources/config.json');

interface ServerResponse {
    data: ServerData
}

interface ServerData {
    id: string
    name: string
}


function buildAPIpath(tableName: string) {
    return 'http://' + CONFIG.dbHost + ':' + CONFIG.dbPort + '/api/' + tableName + '/';
}

export async function getClass(className: string) {
    return await getObjectFromDB(buildAPIpath('class') + '/' + className);
}

export async function getFields(className: string) {
    // console.log(getObjectFromDB(buildAPIpath('field') + '/' + className))
    return await getObjectFromDB(buildAPIpath('field') + '/' + className);

}

export async function getObjects(className: string) {
    // console.log(getObjectFromDB(buildAPIpath('field') + '/' + className))
    return await getObjectFromDB(buildAPIpath('myfield')  + className);

}

export async function getObject(className: string) {
    // console.log(getObjectFromDB(buildAPIpath('field') + '/' + className))
    return await getObjectFromDB(buildAPIpath('myfield') + '/' + className);

}

export function createObject(url: string, payload: object) {

    // const options = {
    //     headers: {"content-type": "application/json"}\
    // }
    console.log(payload);
    const options = {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        data: payload,
        url,
      };
    //   axios(options) .then((response) => {
    //     console.log(response);
    // }, (error) => {
    //     console.log(error);
    // });

    axios.post(url,payload, { headers: { 'content-type': 'application/json' }})
        .then((response) => {
            
            console.log(response);
        }, (error) => {
            console.log(error);
        });
}

export function getObjectFromDB(url: string) {
    // if (process.env.NODE_ENV === process.env.PROD_MODE) {
    url = buildAPIpath(url);
    console.log("here:" + url);
    // return axios.get(url).then(
        
    //     async res => {
    //         let result: any = [];
    //         // console.log(res.data);
    //         result = res.data; 
    //         return result;
    //     });
    // } else {
       return new Promise((r, n) => {
        let result: any = [];

        var var1 = getArgByIndex(url, 0);

        var var2 = getArgByIndex(url, 1);

        if (var1 === "myfield" && var2) {
            result = fakeArray2;
        }
        else if (var1 === "myfield") {
            console.log(var1)
            // result = OrderTestData;
            result = fakeArray2;
        }
        else if (var1 === "fields") {
            // result = fieldMap;
            result = fakeFieldMap;
        }
     /*
         if ( url.includes("myfield") && url.includes("class")) {
              result =  OrderTestData;
          }
          else if ( url.includes("myfield")) {
              result =anOrder;}
        
          else if (url.includes("field")) {
              // result = fieldTestData;
              // result =Object.assign(fieldTestData, fieldDetailTestData);
              result = fieldMap;
             // console.log(result)
          }*/
        if (result) { r(result); }
        else { n([]); }
    });

    // }
}


const order002 = [{ "order date": "2023-2-14", "id": "7b71aa2b-7838-462d-8575-1682704d65e8" }, { "customer": "Apple", "id": "b02fe3ec-7d77-4e3f-8619-d8b5a6808812" }, { "totals": "20000", "id": "cbfb69ad-f600-4e1f-b7f6-9db26c54ca7c" }, { "products": "cables", "id": "99a07e88-f7f1-4cda-8ed7-53b68725d7c7" }, { "note": "please attach delivery notes", "id": "08c43419-de20-4dc2-96b8-2d275d45ac80" }];

const classTestData = [
    { "classId": "9a159df1-f19c-4ed9-b8a3-6a417b7e1279", "className": "purchase" }
    , { "classId": "c834f192-04ee-11ee-8e99-002b67d98d9c", "className": "order" }]

// const fieldTestData = [
// { "fieldId": "5d2446c7-f715-4dde-829f-0f70c97c353b", "classId": "c834f192-04ee-11ee-8e99-002b67d98d9c", "fieldName": "order date" }, 
// { "fieldId": "711cbf51-d237-43b0-b2af-a0833a5ffbbc", "classId": "c834f192-04ee-11ee-8e99-002b67d98d9c", "fieldName": "customer" },
// { "fieldId": "42b361d7-876e-40ea-9178-8a1489ac30b4", "classId": "c834f192-04ee-11ee-8e99-002b67d98d9c", "fieldName": "product" }, 
// { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c496", "classId": "c834f192-04ee-11ee-8e99-002b67d98d9c", "fieldName": "totals" }];

const fieldMap =
{
    "id": "order name",
    "5d2446c7-f715-4dde-829f-0f70c97c353b": "order date",
    "711cbf51-d237-43b0-b2af-a0833a5ffbbc": "customer",
    "42b361d7-876e-40ea-9178-8a1489ac30b4": "product",
    "bff8b823-154f-45b5-82f6-a2e51761c496": "totals",
    "bff8b823-154f-45b5-82f6-a2e51761c498": "product specification",
    "bff8b823-154f-45b5-82f6-a2e51761c497": "product Image",
    //    "bff8b823-154f-45b5-82f6-a2e51761c499": "delivery date",
};

const anOrder =
{
    "id": "23-00002",
    "5d2446c7-f715-4dde-829f-0f70c97c353b": "2023-05-14",
    "711cbf51-d237-43b0-b2af-a0833a5ffbbc": "apple",
    "42b361d7-876e-40ea-9178-8a1489ac30b4": "Bottle Water",
    "bff8b823-154f-45b5-82f6-a2e51761c496": "2000",
    "bff8b823-154f-45b5-82f6-a2e51761c497": '',
    "bff8b823-154f-45b5-82f6-a2e51761c498": "452KR4"
};



const fieldTestData =
    [
        { "fieldId": "id", "fieldName": "order name" },
        { "fieldId": "5d2446c7-f715-4dde-829f-0f70c97c353b", "fieldName": "order date" },
        { "fieldId": "711cbf51-d237-43b0-b2af-a0833a5ffbbc", "fieldName": "customer" },
        { "fieldId": "42b361d7-876e-40ea-9178-8a1489ac30b4", "fieldName": "product" },
        { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c496", "fieldName": "totals" }];

const fieldDetailTestData =
    [{ "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c498", "fieldName": "product specification" },
    { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c497", "fieldName": "product Image" },
    ];

const fakeFieldMap = {
    "id": "order number",
    "uuid-11111": "customer",
    "uuid-11112": {
        "uuid-11113": "product",
        "uuid-11114": "price",
        "uuid-11115": "discount"
    },
    "uuid-11116": "note"
}
const fakeArray = [
    {
        "id": { "uuid-7777": "23-0002" },
        "uuid-11111": { "uuid-12345": "apple market" },
        "uuid-11112": [{ "uuid-11113": { "uuid-12346": "cake" }, "uuid-11114": { "uuid-12333": "300" }, "uuid-11115": { "uuid-12334": "0.1" } }, { "uuid-11113": { "uuid-12350": "pure cake" }, "uuid-11114": { "uuid-12333": "400" }, "uuid-11115": { "uuid-12334": "0.2" } }],

    },
    {
        "id": "23-0001",
        "uuid-11111": { "uuid-12347": "ABC mart" },
        "uuid-11112": [{ "uuid-12348": "cheese cake", "uuid-12351": "560", "uuid-12358": "0" }]
    }
];

const fakeArray2 = [
    {
        "uuid-7777": "23-0002",
        "uuid-12345": "apple market",
        "uuid-12399": [{ "uuid-12346": "cake", "uuid-12333": "300", "uuid-12334": "0.1" }, { "uuid-12350": "pure cake", "uuid-12333": "400", "uuid-12334": "0.2" }],
        "uuid-22226": ""
    },
    {
        "uuid-7778": "23-0001",
        "uuid-12347": "ABC mart",
        "uuid-11112": [{ "uuid-12348": "cheese cake", "uuid-12351": "560", "uuid-12358": "0" }],
        "uuid-22236": "postpone delivery"
    }
];
const OrderTestData = [
    {
        "id": "23-00001",
        "5d2446c7-f715-4dde-829f-0f70c97c353b": "2023-05-14",
        "711cbf51-d237-43b0-b2af-a0833a5ffbbc": "apple",
        "42b361d7-876e-40ea-9178-8a1489ac30b4": "Bottle Water",
        "bff8b823-154f-45b5-82f6-a2e51761c496": "2000"
    },
    {
        "id": "23-00002",
        "5d2446c7-f715-4dde-829f-0f70c97c353b": "2023-05-13",
        "711cbf51-d237-43b0-b2af-a0833a5ffbbc": "apple",
        "42b361d7-876e-40ea-9178-8a1489ac30b4": "Bottle Oranges",
        "bff8b823-154f-45b5-82f6-a2e51761c496": "200"
    },];

const OrderTestData2 = [
    {
        "23-00001": [
            { "fieldId": "5d2446c7-f715-4dde-829f-0f70c97c353b", "fieldValue": "2023-05-14" },
            { "fieldId": "711cbf51-d237-43b0-b2af-a0833a5ffbbc", "fieldValue": "apple" },
            { "fieldId": "42b361d7-876e-40ea-9178-8a1489ac30b4", "fieldValue": "Bottle Water" },
            { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c496", "fieldValue": "2000" }]
    }
    , {
        "23-00002": [
            { "fieldId": "5d2446c7-f715-4dde-829f-0f70c97c353b", "fieldValue": "2023-05-15" },
            { "fieldId": "711cbf51-d237-43b0-b2af-a0833a5ffbbc", "fieldValue": "apple" },
            { "fieldId": "42b361d7-876e-40ea-9178-8a1489ac30b4", "fieldValue": "Oranges" },
            { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c496", "fieldValue": "300" }]
    }
]


const anOrderA =
    [{ "fieldId": "5d2446c7-f715-4dde-829f-0f70c97c353a", "fieldValue": "23-00002" },
    { "fieldId": "5d2446c7-f715-4dde-829f-0f70c97c353b", "fieldValue": "2023-05-14" },
    { "fieldId": "711cbf51-d237-43b0-b2af-a0833a5ffbbc", "fieldValue": "apple" },
    { "fieldId": "42b361d7-876e-40ea-9178-8a1489ac30b4", "fieldValue": "Bottle Water" },
    { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c496", "fieldValue": "2000" },
    { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c497", "fieldValue": '' },
    { "fieldId": "bff8b823-154f-45b5-82f6-a2e51761c498", "fieldValue": "452KR4" }];



//const Example = ({ data }) => <img src={`data:image/jpeg;base64,${data}`} />